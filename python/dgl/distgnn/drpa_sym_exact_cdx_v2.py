...
class drpa_master(DGLHeteroGraph):
    ...
    def update_all(self,
        ...
        enable = True
        if enable or (epoch == 0 or (epoch > 0 and self.epochi > 0)):  ## pcl
            self.r_in_degs = DGLHeteroGraph.in_degrees(self).to(feat_dst)
            if self.nrounds != -1:
        ...
        ### 
        ### if epoch == 0 and self.epochi == 0: ##pcl
        ...
        if (not enable) and (epoch == 0 and self.epochi == 0): ##pcl
            self.buf_neigh = self.dstdata['neigh'].detach().clone()
        #######################
    ...
    def drpa_create_buckets_v4(self):
        ...
        self.selected_nodes_bck[0] = self.L[0].tolist()
        self.selected_nodes_bck[1] = self.L[0].tolist()
        self.selected_nodes_bck[2] = self.L[0].tolist()
        #self.selected_nodes_bck[0] = nid.tolist()
        #self.selected_nodes_bck[1] = nid.tolist()
        #self.selected_nodes_bck[2] = nid.tolist()
