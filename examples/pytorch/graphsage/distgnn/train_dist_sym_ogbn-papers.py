...
from dgl.distgnn.drpa_sym_exact_cdx_mfg import drpa as drpa_mfg
...
def run(g, data):
    ...
    if args.resume and os.path.isfile(args.resume_file):
        print("=> loading model & optimizer checkpoint '{}'".format(args.resume_file))
        checkpoint = th.load(args.resume_file)
        args.start_epoch = checkpoint['epoch']
        model.module.load_state_dict(checkpoint['state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer'])
        print("=> loaded model & optimizer checkpoint '{}' (epoch {})".format(args.resume_file, checkpoint['epoch']))
    ...
    for epoch in range(args.start_epoch, args.n_epochs):            
        ...
        toc = time.time()
        if args.val:
            dnr, ddr = evaluate_dist(model, g, features, labels, val_nid)
            dnr_t = th.tensor(dnr, dtype=th.int32)
            ddr_t = th.tensor(ddr, dtype=th.int32)
            dist.all_reduce(dnr_t, op=th.distributed.ReduceOp.SUM)
            dist.all_reduce(ddr_t, op=th.distributed.ReduceOp.SUM)
            acc_val = float(dnr_t*1.0/ddr_t)
            acc_train = 0
        ...
        if (epoch %5 == 0 or epoch == args.n_epochs - 1):
            dnr, ddr = evaluate_dist(model, g, features, labels, test_nid)
            dnr_t = th.tensor(dnr, dtype=th.int32)
            ddr_t = th.tensor(ddr, dtype=th.int32)
            dist.all_reduce(dnr_t, op=th.distributed.ReduceOp.SUM)
            dist.all_reduce(ddr_t, op=th.distributed.ReduceOp.SUM)
            dacc = float(dnr_t*1.0/ddr_t)
                        
            if args.rank == 0:
                if dbest_acc < dacc: dbest_acc = dacc
                print("#############################################################", flush=True)
                print("Dist accuracy: Avg: {:0.4f}, best_acc: {:.4f}".
                      format(dacc, dbest_acc), flush=True)
                print("############ ddr, #original test nodes: ", ddr_t)
                #print("Acc diff: {:.4f}".format(acc - dacc))
                print("#############################################################", flush=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--start-epoch", type=int, default=0,
                        help="start number of training epochs")    
    ...
    parser.add_argument("--resume-file", type=str, default="ogbn-papers16.model",
                        help="Model file name to load")
    parser.add_argument("--checkpoint-file", type=str, default="ogbn-papers16.model",
                        help="model file")
    ...
    n_classes = 172  ## need to shift this to partition json
    if args.rank == 0:
